  ...
  return (
    <div className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12 lg:pl-24">
      <div className="absolute top-0 left-0 h-[95vh] w-screen -z-10">
        <Image
          src={`${baseUrl}/${
            createRandomMedias?.backdrop_path || createRandomMedias?.poster_path
          }`}
          alt="Banner"
          layout="fill"
          objectFit="cover"
      ...
      <h1 className="text-2xl md:text-4xl lg:text-7xl font-bold">
        {createRandomMedias?.title ||
          createRandomMedias?.name ||
          createRandomMedias?.original_name}
      </h1>
      <p className="max-w-xs text-shadow-md text-xs md:max-w-lg md:text-lg lg:max-w-2xl line-clamp-5">
        {createRandomMedias?.overview}
      </p>
      <div className="flex space-x-3">
        <button
          onClick={() =>
            router.push(
              `/watch/${createRandomMedias?.type}/${createRandomMedias?.id}`
            )
          }
